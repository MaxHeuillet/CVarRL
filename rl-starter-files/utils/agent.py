...
class Agent:
    ...
    def __init__(self, obs_space, action_space, model_dir, type = '2m',
    ...
    def get_actions(self, obss, stochasticity):
        ...
        new_dist = torch.tensor( [ self.stochasticity(act, float(stochasticity) ) for act in init_actions], device = 'cuda' )

        new_dist = Categorical(probs = new_dist)

        actions = new_dist.sample()

        return actions.cpu().numpy()

    def get_action(self, obs, stochasticity):
        return self.get_actions(obs, stochasticity)[0]
...
class AdversaryAgent:
    ...
    def get_perturbations(self, obss):
        ...
        with torch.no_grad():
            if self.acmodel.recurrent:
                _, dist, _, self.memories = self.acmodel( preprocessed_obss, self.memories )
            else:
