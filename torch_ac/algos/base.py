...
class BaseAlgo(ABC):
    ...
    def __init__(self, envs, acmodel, acmodel_adversary, pbt, device, num_frames_per_proc, discount, lr, gae_lambda, entropy_coef,
    ...
    def collect_experiences(self):
        ...
        for i in range(self.num_frames_per_proc): #  
            ...
            with torch.no_grad():
                ...
                state_dist = torch.tensor( [ self.stochasticity(act, self.pbt) for act in init_action], device =agent_pos.device, dtype=agent_pos.dtype )
            ...
            obs, reward, done, _ = self.env.step( action.cpu().numpy(), adv_dist.probs.cpu().numpy(), state_dist.cpu().numpy(), None, None )
            ...
            if self.reshape_reward is not None:
                self.rewards[i] = torch.tensor([ self.reshape_reward(obs_, action_, reward_, done_)
                            for obs_, action_, reward_, done_ in zip(obs, action, reward, done)], device=self.device)
        ...
        exps.inputs_adversary =  torch.tensor( [ self.inputs_adversary[i][j].cpu().numpy() for j in range(self.num_procs) for i in range(self.num_frames_per_proc) ], device = self.device)
        exps.budgets =  torch.tensor( [ self.budgets[i][j] for j in range(self.num_procs) for i in range(self.num_frames_per_proc) ], device = self.device)
